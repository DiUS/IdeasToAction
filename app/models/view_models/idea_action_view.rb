class IdeaActionView
  ...
  def initialize
    ...
    random_recent = IdeaAction.recent.excluding_idea_actions(already_selected.collect(&:id)).random(2)
    self.recent = random_recent
    already_selected.concat random_recent

    self.popular =  IdeaAction.popular.excluding_idea_actions(already_selected.collect(&:id)).random(2)
  end
end